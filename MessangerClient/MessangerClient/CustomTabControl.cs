using System;
using System.Drawing;
using System.Runtime.InteropServices;
using System.Windows.Forms;
using System.Drawing.Drawing2D;

namespace MessangerClient
{
    public partial class CustomTabControl : System.Windows.Forms.TabControl
    {
        public event TabClosedEventHendler TabPageClosed;

        private int _hotTabIndex = -1;
        private Color closeButtonXColor = Color.White;
        private Color closeButtonColor = Color.Red;
        private SolidBrush activTabBrush = new SolidBrush(Color.White);
        private Color disTabColor1 = Color.White;
        private Color disHlTabColor = Color.Orange;
        private Color disTabColor2 = Color.LightGray;
        private Color hotTabColor1 = Color.LightBlue;
        private Color hotTabColor2 = Color.LightGray;
        private Pen disTabBorderPen = new Pen(Color.Gray);

        public CustomTabControl()
            : base()
        {
            this.SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw | ControlStyles.UserPaint, true);
        }

        #region Properties

        private int CloseButtonHeight
        {
            get { return FontHeight; }
        }

        private int HotTabIndex
        {
            get { return _hotTabIndex; }
            set
            {
                if (_hotTabIndex != value)
                {
                    _hotTabIndex = value;
                    this.Invalidate();
                }
            }
        }

        #endregion

        #region Overridden Methods

        protected override void OnCreateControl()
        {
            base.OnCreateControl();
            this.OnFontChanged(EventArgs.Empty);
        }

        protected override void OnFontChanged(EventArgs e)
        {
            base.OnFontChanged(e);
            IntPtr hFont = this.Font.ToHfont();
            SendMessage(this.Handle, WM_SETFONT, hFont, new IntPtr(-1));
            SendMessage(this.Handle, WM_FONTCHANGE, IntPtr.Zero, IntPtr.Zero);
            this.UpdateStyles();
        }

        protected override void OnMouseMove(MouseEventArgs e)
        {
            base.OnMouseMove(e);
            TCHITTESTINFO HTI = new TCHITTESTINFO(e.X, e.Y);
            HotTabIndex = SendMessage(this.Handle, TCM_HITTEST, IntPtr.Zero, ref HTI);
        }

        protected override void OnMouseLeave(EventArgs e)
        {
            base.OnMouseLeave(e);
            HotTabIndex = -1;
        }


        protected override void OnPaintBackground(PaintEventArgs pevent)
        {
            base.OnPaintBackground(pevent);
            if (pevent.ClipRectangle.Equals(this.ClientRectangle))
                DrawControlBackground(pevent.Graphics, pevent.ClipRectangle);
            else
                this.Invalidate();
            for (int id = 0; id < this.TabCount; id++)
                DrawTabBackground(pevent.Graphics, id);
        }

        protected override void OnPaint(PaintEventArgs e)
        {
            base.OnPaint(e);
            for (int id = 0; id < this.TabCount; id++)
                DrawTabContent(e.Graphics, id);
        }

        protected override void OnSelected(TabControlEventArgs e)
        {
            base.OnSelected(e);
            TabPage tp = e.TabPage;
            if (tp != null && tp.BackColor == disHlTabColor)
                UnHighlightTabPage(e.TabPageIndex);
        }

        protected override void WndProc(ref Message m)
        {
            if (m.Msg == TCM_SETPADDING)
                m.LParam = MAKELPARAM(this.Padding.X + CloseButtonHeight / 2, this.Padding.Y);

            if (!this.DesignMode && HotTabIndex >= 0)
            {
                if (m.Msg == WM_LBUTTONDOWN)
                {
                    Point pt = this.PointToClient(Cursor.Position);
                    Rectangle closeRect = GetCloseButtonRect(HotTabIndex);
                    if (closeRect.Contains(pt))
                    {
                        CloseTab(HotTabIndex);
                        m.Msg = WM_NULL;
                    }
                }
                else if (m.Msg == WM_MBUTTONDOWN)
                {
                    Point pt = this.PointToClient(Cursor.Position);
                    Rectangle tabRect = GetTabRect(HotTabIndex);
                    if (tabRect.Contains(pt))
                    {
                        CloseTab(HotTabIndex);
                        m.Msg = WM_NULL;
                    }
                }
            }
            base.WndProc(ref m);
        }

        #endregion

        protected virtual void OnTabeClosed(TabClosedEventArgs e)
        {
            TabClosedEventHendler handler = TabPageClosed;
            if (handler != null)
            {
                handler(this, e);
            }
        }

        #region Public Methods

        public void UnHighlightTabPage(TabPage page)
        {
            if (page != null)
            {
                page.BackColor = disTabColor1;
                this.Invalidate();
            }
        }

        public void UnHighlightTabPage(int index)
        {
            if (index != -1 && index < TabCount)
            {
                UnHighlightTabPage(TabPages[index]);
            }
        }

        public void UnHighlightTabPage(string key)
        {
            int i = TabPages.IndexOfKey(key);
            if (i != -1)
                UnHighlightTabPage(i);
        }

        public void HighlightTabPage(int index)
        {
            if (index != -1 && index < TabCount)
            {
                TabPage page = TabPages[index];
                if (page != null && SelectedIndex != index)
                {
                    page.BackColor = disHlTabColor;
                    this.Invalidate();
                }
            }
        }

        public void HighlightTabPage(string key)
        {
            int i = TabPages.IndexOfKey(key);
            HighlightTabPage(i);
        }

        #endregion

        #region Private Methods

        private void CloseTab(int i)
        {
            TabPage page = TabPages[i];
            TabPages.Remove(page);
            if (TabPages.Count > i)
                SelectTab(i);
            else
            {
                i--;
                if (i >= 0 && TabPages.Count > i)
                    SelectTab(i);
            }
            OnTabeClosed(new TabClosedEventArgs(page));
        }

        private IntPtr MAKELPARAM(int lo, int hi)
        {
            return new IntPtr((hi << 16) | (lo & 0xFFFF));
        }

        private void DrawControlBackground(Graphics g, Rectangle r)
        {
            if (TabCount > 0)
            {
                r.Offset(2, 20);
                r.Width -= 4;
                r.Height -= 22;
            }
            else
            {
                r.Offset(2, 2);
                r.Width -= 4;
                r.Height -= 4;
            }
            g.FillRectangle(Brushes.White, r);
            g.DrawRectangle(disTabBorderPen, r);
        }

        private void DrawTabBackground(Graphics graphics, int id)
        {
            Rectangle rc = GetTabRect(id);
            GraphicsPath bPath;
            if (id == SelectedIndex)
            {
                rc.Height += 1;
                bPath = GetTabBorder(rc);
                graphics.FillPath(activTabBrush, bPath);
            }
            else
            {
                Color gc1;
                Color gc2;
                if (id == HotTabIndex)
                {
                    gc1 = hotTabColor1;
                    gc2 = hotTabColor2;
                }
                else
                {
                    gc1 = TabPages[id].BackColor == disHlTabColor ? disHlTabColor : disTabColor1;
                    gc2 = disTabColor2;
                }
                LinearGradientBrush lgb = new LinearGradientBrush(
                        rc, gc1, gc2, LinearGradientMode.Vertical);
                bPath = GetTabBorder(rc);
                graphics.FillPath(lgb, bPath);
            }
            graphics.DrawPath(disTabBorderPen, bPath);
        }

        private GraphicsPath GetTabBorder(Rectangle r)
        {
            GraphicsPath bPath = new GraphicsPath();
            int cornerSize = 7;
            bPath.AddLine(r.X, r.Y + r.Height, r.X, r.Y + cornerSize);
            bPath.AddArc(r.X, r.Y, cornerSize, cornerSize, 180, 90);
            bPath.AddLine(r.X + cornerSize, r.Y, r.X + r.Width - cornerSize, r.Y);
            bPath.AddArc(r.X + r.Width - cornerSize, r.Y, cornerSize, cornerSize, 270, 90);
            bPath.AddLine(r.X + r.Width, r.Y + cornerSize, r.X + r.Width, r.Y + r.Height);
            return bPath;
        }

        private void DrawTabContent(Graphics graphics, int id)
        {
            bool selectedOrHot = id == this.SelectedIndex || id == this.HotTabIndex;
            bool vertical = this.Alignment >= TabAlignment.Left;

            Image tabImage = null;

            if (this.ImageList != null)
            {
                TabPage page = this.TabPages[id];
                if (page.ImageIndex > -1 && page.ImageIndex < this.ImageList.Images.Count)
                    tabImage = this.ImageList.Images[page.ImageIndex];

                if (page.ImageKey.Length > 0 && this.ImageList.Images.ContainsKey(page.ImageKey))
                    tabImage = this.ImageList.Images[page.ImageKey];
            }

            Rectangle tabRect = GetTabRect(id);
            Rectangle contentRect = vertical ? new Rectangle(0, 0, tabRect.Height, tabRect.Width) : new Rectangle(Point.Empty, tabRect.Size);
            Rectangle textrect = contentRect;
            textrect.Width -= FontHeight;

            if (tabImage != null)
            {
                textrect.Width -= tabImage.Width;
                textrect.X += tabImage.Width;
            }

            Color frColor = Color.Black;

            using (Bitmap bm = new Bitmap(contentRect.Width, contentRect.Height))
            {
                using (Graphics bmGraphics = Graphics.FromImage(bm))
                {
                    TextRenderer.DrawText(bmGraphics, this.TabPages[id].Text, this.Font, textrect, frColor);
                    if (selectedOrHot)
                    {
                        Rectangle closeRect = new Rectangle(contentRect.Right - CloseButtonHeight, 0, CloseButtonHeight, CloseButtonHeight);
                        closeRect.Offset(-2, (contentRect.Height - closeRect.Height) / 2);
                        DrawCloseButton(bmGraphics, closeRect);
                    }
                    if (tabImage != null)
                    {
                        Rectangle imageRect = new Rectangle(Padding.X, 0, tabImage.Width, tabImage.Height);
                        imageRect.Offset(0, (contentRect.Height - imageRect.Height) / 2);
                        bmGraphics.DrawImage(tabImage, imageRect);
                    }
                }

                if (vertical)
                {
                    if (this.Alignment == TabAlignment.Left)
                        bm.RotateFlip(RotateFlipType.Rotate270FlipNone);
                    else
                        bm.RotateFlip(RotateFlipType.Rotate90FlipNone);
                }
                graphics.DrawImage(bm, tabRect);

            }
        }

        private void DrawCloseButton(Graphics graphics, Rectangle bounds)
        {
            graphics.FillRectangle(Brushes.Red, bounds);
            using (Font closeFont = new Font("Arial", Font.Size, FontStyle.Bold))
                TextRenderer.DrawText(graphics, "X", closeFont, bounds, Color.White, Color.Red, TextFormatFlags.HorizontalCenter | TextFormatFlags.NoPadding | TextFormatFlags.SingleLine | TextFormatFlags.VerticalCenter);

        }

        private Rectangle GetCloseButtonRect(int id)
        {
            Rectangle tabRect = GetTabRect(id);
            Rectangle closeRect = new Rectangle(tabRect.Left, tabRect.Top, CloseButtonHeight, CloseButtonHeight);

            switch (Alignment)
            {
                case TabAlignment.Left:
                    closeRect.Offset((tabRect.Width - closeRect.Width) / 2, 0);
                    break;
                case TabAlignment.Right:
                    closeRect.Offset((tabRect.Width - closeRect.Width) / 2, tabRect.Height - closeRect.Height);
                    break;
                default:
                    closeRect.Offset(tabRect.Width - closeRect.Width, (tabRect.Height - closeRect.Height) / 2);
                    break;
            }

            return closeRect;
        }

        #endregion

        #region Interop

        [DllImport("user32.dll")]
        private static extern IntPtr SendMessage(IntPtr hWnd, int msg, IntPtr wParam, IntPtr lParam);

        [DllImport("user32.dll")]
        private static extern int SendMessage(IntPtr hwnd, int msg, IntPtr wParam, ref TCHITTESTINFO lParam);

        [StructLayout(LayoutKind.Sequential)]
        private struct TCHITTESTINFO
        {
            public Point pt;
            public TCHITTESTFLAGS flags;
            public TCHITTESTINFO(int x, int y)
            {
                pt = new Point(x, y);
                flags = TCHITTESTFLAGS.TCHT_NOWHERE;
            }
        }

        [Flags()]
        private enum TCHITTESTFLAGS
        {
            TCHT_NOWHERE = 1,
            TCHT_ONITEMICON = 2,
            TCHT_ONITEMLABEL = 4,
            TCHT_ONITEM = TCHT_ONITEMICON | TCHT_ONITEMLABEL
        }

        private const int WM_NULL = 0x0;
        private const int WM_SETFONT = 0x30;
        private const int WM_FONTCHANGE = 0x1D;
        private const int WM_LBUTTONDOWN = 0x202;
        private const int WM_MBUTTONDOWN = 0x208;

        private const int TCM_FIRST = 0x1300;
        private const int TCM_HITTEST = TCM_FIRST + 13;
        private const int TCM_SETPADDING = TCM_FIRST + 43;

        #endregion

    }

    public delegate void TabClosedEventHendler(object sender, TabClosedEventArgs e);
    public class TabClosedEventArgs : EventArgs
    {
        public TabClosedEventArgs(TabPage page)
        {
            p = page;
        }
        private TabPage p;
        public TabPage Page
        {
            get { return p; }
        }
    }
}
